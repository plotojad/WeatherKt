...
import kotlinx.android.synthetic.main.addinfodialog.*
...
class AddInfoDialogFragment(val mView: MainContract.View) : DialogFragment() {
    ...
    private lateinit var spinnTypeAdd: Spinner
    ...
    override fun onCreateView(
        ...
        viewsMap = mapOf(
            "name" to etCityNameAdd,
            "decTemp" to etDecTempAdd,
            "janTemp" to etJanTempAdd,
            "febTemp" to etFebTempAdd,
            "marTemp" to etMarTempAdd,
            "aprTemp" to etAprTempAdd,
            "mayTemp" to etMayTempAdd,
            "junTemp" to etJunTempAdd,
            "julTemp" to etJulTempAdd,
            "augTemp" to etAugTempAdd,
            "sepTemp" to etSeptTempAdd,
            "ocTemp" to etOctTempAdd,
            "novTemp" to etNovTempAdd
        )

        val adapter = ArrayAdapter(context!!, android.R.layout.simple_spinner_item, types)
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinnTypeAdd.adapter = adapter
        spinnTypeAdd.onItemSelectedListener = object : AdapterView.OnItemSelectedListener {
        ...
        btAddInfo.setOnClickListener(View.OnClickListener {
            ...
            Toast.makeText(context, resources.getString(R.string.cityIsAdded), Toast.LENGTH_SHORT)
                .show()
            dismiss()
        })
        return v
